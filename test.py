...
parser.add_argument('--n_layer', type=int, default=2, help='num of rwkv')
...
parser.add_argument('--pretrain', type=str, default="/mnt/data2/Mutil_data/Visual-Selective-VIO-main/results/experiment/checkpoints/080_2.pth", help='path to the pretrained model')
...
def train(model, optimizer, train_loader, logger, ep, weighted=False):
    ...
    for i, (imgs, imus, gts, rot, weight) in enumerate(train_loader):
        ...
        loss = (pose_loss).to(torch.bfloat16)
        loss.backward()
        optimizer.step()
